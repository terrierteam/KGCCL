...
def train_contrast(recommend_model, contrast_model, contrast_views, optimizer):
    ...
    recmodel.train()
    aver_loss = 0.
    uiev1, uiev2 = contrast_views["uiev1"], contrast_views["uiev2"]
    uiv1, uiv2 = contrast_views["uiv1"], contrast_views["uiv2"]
    l_kg = list()
    l_item = list()
    l_user = list()
    if world.kgc_enable:
        kgv1_readouts = recmodel.cal_item_embedding_from_kg(uiev1).split(2048)
        kgv2_readouts = recmodel.cal_item_embedding_from_kg(uiev2).split(2048)
...
def BPR_train_contrast(dataset, recommend_model, loss_class, contrast_model :Contrast, contrast_views, epoch, optimizer, neg_k=1, w=None):
    ...
    for batch_i, train_data in tqdm(enumerate(dataloader), total=len(dataloader),disable=True):
        ...
        if world.uicontrast!="NO":
            if world.kgc_joint:
                _, _, usersv1_ro, itemsv1_ro = Recmodel.view_computer(uiv1, uiev1)
                _, _, usersv2_ro, itemsv2_ro = Recmodel.view_computer(uiv2, uiev2) 
            else:
